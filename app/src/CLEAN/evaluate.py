...
def maximum_separation(dist_lst, first_grad, use_max_grad):
    ...
    # 将数组 dist_lst 的除第一个元素外的所有元素都保留，然后在末尾附加 10 个重复的最后一个元素
    gamma = np.append(dist_lst[1:], np.repeat(dist_lst[-1], 10))
    # print(f'gamma:{gamma}')
    # gamma:[0.05293718 0.06757995 0.10624887 0.10870343 0.13690375 0.18738529
    # 0.18867664 0.19719008 0.19742492 0.19742492 0.19742492 0.19742492
    # 0.19742492 0.19742492 0.19742492 0.19742492 0.19742492 0.19742492
    # 0.19742492]
    sep_lst = np.abs(dist_lst - np.mean(gamma))
    # print(f'sep_lst:{sep_lst}')
    # sep_lst:[0.16932801 0.11639436 0.10175159 0.06308267 0.06062811 0.03242779
    # 0.01805375 0.0193451  0.02785853 0.02809338]

    sep_grad = np.abs(sep_lst[:-1]-sep_lst[1:])
    # print(f'sep_grad:{sep_grad}')
    ...
    if use_max_grad:
        ...
        # print(f'large_grads:{large_grads}')
        max_sep_i = large_grads[-1][opt]
        # print(f'max_sep_i:{max_sep_i}')
...
def write_max_sep_choices(df, csv_name, first_grad=False, use_max_grad=False, gmm = None):
    ...
    for col in df.columns:  
        ...
        for i in range(max_sep_i+1):
            ...
            dist_i = dist_lst[i] 
            print(f'EC_i:{EC_i}, dist_i:{dist_i}')
            if gmm != None:
